    ...
    public class ScanApproveSendService : IScanApproveSendService
    {
        public ScanApproveFinView ScanApvSendAdd(ScanApproveAddView model)
        {
            using (var ctx = new ConXContext())
            {
                ...
                {
                    string sql1 = "select entity ,tran_no , tran_date , wc_code  , prod_code , bar_code , to_char(fin_date,'dd/mm/yyyy') fin_date from pkg_barcode where entity = :p_entity and trunc(fin_date) = to_date(:p_fin_date,'dd/mm/yyyy') and pkg_barcode_set = :p_set_no";
                    SetDataView set_no = ctx.Database.SqlQuery<SetDataView>(sql1, new OracleParameter("p_entity", ventity), new OracleParameter("p_fin_date", vfin_date), new OracleParameter("p_det_no", vset_no)).FirstOrDefault();
                    ...
                    using (TransactionScope scope = new TransactionScope())
                    {
                        ...
                        string sql6 = "select prod_code from pd_det where pd_entity = :p_entity and doc_no = :p_doc_no and por_no = :p_por_no";
                        string chkdup = ctx.Database.SqlQuery<string>(sql6, new OracleParameter("p_entity", mps.po_entity), new OracleParameter("p_doc_no", vdoc_no), new OracleParameter("p_prod_code", mps.prod_code)).FirstOrDefault();
                        ...
                        else
                        {
                            ...
                            oraCommandpd.BindByName = true;
                            oraCommandpd.Parameters.AddRange(parampd);
                            oraCommandpd.CommandText = "update pd_det set qty_pdt = qty_pdt + :p_qty where pd_entity = :p_entity and doc_no = :p_doc_no and por_no = :p_por_no";
                        ...
                        oraCommandmps.CommandText = "update mps_det_wc set doc_status = 'P' , doc_no = :p_doc_no , gen_by = :p_gen_by , gen_date = sysdate where pd_entity = :p_entity and prod_code = :p_prod_code and por_no = :p_por_no and wc_code = :p_wc_code and trunc(fin_date) = to_date(:p_fin_date,'dd/mm/yyyy') and gen_date is null and rownum <= :p_qty";
            ...
            using (var ctx = new ConXContext())
            {
                ...
                if (vbuild_type == "HMJIT")
                {
                    ...
                    using (TransactionScope scope = new TransactionScope())
                    {
                        ...
                        oraCommandmps.BindByName = true;
                        oraCommandmps.Parameters.AddRange(parammps);
                        oraCommandmps.CommandText = "update mps_det_wc set doc_status = 'P' , doc_no = :p_doc_no , gen_by = :p_gen_by , gen_date = sysdate where entity = :p_entity and prod_code = :p_prod_code and por_no = :p_por_no and wc_code = :p_wc_code and trunc(fin_date) = to_date(:p_fin_date,'dd/mm/yyyy') and gen_date is null and rownum <= :p_qty";
