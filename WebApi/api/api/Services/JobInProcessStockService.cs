        ...
        public JobInProcessStockView EntryAdd(JobInProcessStockSearchView model)
        {
            using (var ctx = new ConXContext())
            {
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    ...
                    oraCommand.BindByName = true;
                    oraCommand.Parameters.AddRange(param);
                    oraCommand.CommandText = "update mps_det_wc_stk set qty_fin = qty_fin + :p_qty , fin_by =:p_fin_by , fin_date = SYSDATE where entity = :p_entity  and wc_code =:p_wc_code and por_no = :p_por_no and ref_no = :p_ref_no and prod_code = :p_prod_code and prod_code_sub = :p_prod_code_sub";
